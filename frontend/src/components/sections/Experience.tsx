      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="space-y-8">
          {experience.map((item, idx) => (
            <div
              key={idx}
              className="glass-card rounded-3xl p-6 sm:p-8 border border-slate-200/80 hover:border-blue-500/40 transition-all duration-300 relative shadow-lg shadow-slate-200/50 bg-white/90"
            >
              {/* Header Info */}
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200 pb-6 mb-6">
                <div>
                  ...
                  <p className="font-mono text-xs text-blue-600 font-semibold mt-1">{item.company}</p>
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {item.responsibilities.map((resp, i) => (
                    <div key={i} className="flex items-start gap-2.5 text-xs sm:text-sm text-slate-700 leading-relaxed rounded-xl bg-slate-50 p-3 border border-slate-200/80">
